...
REGISTER_OP("SparseEdgeConvolutionGrad")
    .Input("grad: float32")
    .Input("v_in: float32")
   ...
   // .Attr("num_layers: int")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
      ...
      // Validate shapes
      TF_RETURN_IF_ERROR(c->WithRank(c->input(0), 1, &grad_shape)); // #Edge vals
	  ...
	  TF_RETURN_IF_ERROR(c->WithRank(c->input(4), 2, &h_shape)); // h = LxF1xF2
      ...
      ShapeHandle grad_h = c->MakeShape({c->Dim(h_shape, 0), c->Dim(h_shape, 1)});
...
class SparseEdgeConvolutionGradOp : public OpKernel {
 public:
  explicit SparseEdgeConvolutionGradOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES_OK(context, context->allocate_output(3, grad_h_weights_shape,
                                                     &grad_h_weights));
    auto grad_h_weights_vals = grad_h_weights->tensor<T, 2>();
    ...
    auto threadFunc = [&](int thread_id) {
        ...
        for (int64 edge = thread_id; edge < num_edges; edge+=num_threads)
        {
            ...
            {
                currentWeight = h(2*in_features,filter);
                grad_a_values_vals(edge) += currentGrad*currentWeight;
                grad_h_weights_vals(2*in_features,filter) += currentGrad*currentVal;
            }
        }
    };
    ...
    const int num_threads2 = std::min(thread_pool->num_threads, num_edges);
    auto threadFunc2 = [&](int thread_id) {
        ...
        T currentVertexS;
        T currentVertexR;
        for (int64 edge = thread_id; edge < num_edges; edge+=num_threads2)
        {
            ...
            for(int64 filter = 0; filter < num_filters; filter++)
            {
                for(int64 f1 = 0; f1 < in_features; f1++)
                {
                    ...
                    grad_h_weights_vals(f1,filter) += currentGrad*currentVertexS;
                    grad_h_weights_vals(in_features + f1,filter) += currentGrad*currentVertexR;
                    grad_v_in_vals(currentBatch,currentN1,f1) += currentGrad*currentWeightS;
                    grad_v_in_vals(currentBatch,currentN2,f1) += currentGrad*currentWeightR;
                }
            }
        }
    };
...
REGISTER_KERNEL_BUILDER(Name("SparseEdgeConvolutionGrad").Device(DEVICE_CPU), SparseEdgeConvolutionGradOp<CPUDevice, T>);
...
#define REGISTER_GPU_KERNEL(T)                                     \
  REGISTER_KERNEL_BUILDER(Name("SparseEdgeConvolutionGrad") \
                              .Device(DEVICE_GPU),          \
                          SparseEdgeConvolutionGradOp<GPUDevice, T>);
