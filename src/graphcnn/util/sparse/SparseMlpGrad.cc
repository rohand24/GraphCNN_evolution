...
bool SparseMLPGradLauncher(const float * g,
                        const int64 * indices,
                        const float * values,
                        const float * vertices,
                        float * grad_v_in_vals,
                        float * grad_h_values_vals,
                        const int num_edges,
...
REGISTER_OP("SparseMlpGrad")
    .Input("grad: float32")
    .Input("v_in: float32")
    .Input("h_indices: int64")
    .Input("h_values: float32")
    .Input("new_v_shape: int64")
   // .Attr("num_layers: int")
    .Output("grad_v_in: float32")
    .Output("grad_h_indices: int64")
    .Output("grad_h_values: float32")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
      ...
      ShapeHandle grad_h_indices = c->MakeShape({c->Dim(idx_shape, 0), c->Dim(idx_shape, 1)});
      ShapeHandle grad_h_values = c->MakeShape({c->Dim(values_shape, 0)});
      ShapeHandle grad_v_new_shape = c->MakeShape({c->Dim(v_new_shape, 0)});

      c->set_output(0, grad_v_in);
      c->set_output(1, grad_h_indices);
      c->set_output(2, grad_h_values);
      c->set_output(3, grad_v_new_shape);
      return Status::OK();
    });

template <typename Device, typename T>
class SparseMLPGradOp : public OpKernel {
 public:
  explicit SparseMLPGradOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& H_indices = context->input(2);
    const Tensor& H_values = context->input(3);
	const Tensor& V_new_shape = context->input(4);

    auto g = grad.tensor<T,3>();
    auto vertices = Vin.tensor<T, 3>();
    auto indices = H_indices.tensor<int64, 2>();
    auto values = H_values.tensor<T, 1>();
    ...
    TensorShape grad_h_indices_shape;
    grad_h_indices_shape.AddDim(H_indices.dim_size(0));
    grad_h_indices_shape.AddDim(H_indices.dim_size(1));
    Tensor* grad_h_indices = NULL;
    OP_REQUIRES_OK(context, context->allocate_output(1, grad_h_indices_shape,
                                                     &grad_h_indices));
    auto grad_h_indices_vals = grad_h_indices->tensor<int64, 2>();
    auto grad_h_indices_vals_flat = grad_h_indices->flat<int64>();

    TensorShape grad_h_values_shape;
    grad_h_values_shape.AddDim(H_values.dim_size(0));
    Tensor* grad_h_values = NULL;
    OP_REQUIRES_OK(context, context->allocate_output(2, grad_h_values_shape,
                                                     &grad_h_values));
    auto grad_h_values_vals = grad_h_values->tensor<T, 1>();
    auto grad_h_values_vals_flat = grad_h_values->flat<T>();
    ...
    ConstOutLauncher<int64>(grad_h_indices_vals_flat.data(),
                    grad_h_indices_vals_flat.size(),
                    0,
                    context->eigen_gpu_device());

    ConstOutLauncher<T>(grad_h_values_vals_flat.data(),
                    grad_h_values_vals_flat.size(),
                    0,
                    context->eigen_gpu_device());
    ...
    const int64 num_unwrapped_features_h_indices = grad_h_indices_vals_flat.size();
    for (int64 i = 0; i < num_unwrapped_features_h_indices; i++) {
      grad_h_indices_vals_flat(i) = 0;
    }

    const int64 num_unwrapped_features_h_values = grad_h_values_vals_flat.size();
    for (int64 i = 0; i < num_unwrapped_features_h_values; i++) {
      grad_h_values_vals_flat(i) = 0;
    }
    ...
    const int num_edges = H_indices.dim_size(0);
    const int out_num_vertices = shape(1);

    #if GOOGLE_CUDA

    SparseMLPGradLauncher(grad.flat<T>().data(),
                        H_indices.flat<int64>().data(),
                        H_values.flat<T>().data(),
                        Vin.flat<T>().data(),
                        grad_v_in_vals_flat.data(),
                        grad_h_values_vals_flat.data(),
                        num_edges,
                        in_features,
                        num_vertices,
                        out_num_vertices,
	...
	auto threadFunc = [&](int thread_id) {
        ...
        {
            for (int currentBatch = 0; currentBatch < batch_size; currentBatch++)
            {
                currentN1 = indices(edge,0);
                currentN2 = indices(edge,1);
                ...
                for(int64  f1 = 0; f1 < in_features; f1++)
                {
                    ...
                    grad_h_values_vals(edge) += currentGrad*currentVertex;
...
REGISTER_KERNEL_BUILDER(Name("SparseMlpGrad").Device(DEVICE_CPU), SparseMLPGradOp<CPUDevice, T>);
...
#define REGISTER_GPU_KERNEL(T)                                     \
  REGISTER_KERNEL_BUILDER(Name("SparseMlpGrad").Device(DEVICE_GPU).HostMemory("new_v_shape"),          \
                          SparseMLPGradOp<GPUDevice, T>);
