...
REGISTER_OP("SparseMaxVertexPool")
    .Input("v_in: float32")
...
class SparseMaxVertexPoolOp : public OpKernel {
 public:
  explicit SparseMaxVertexPoolOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    ConstOutLauncher<T>(output_flat.data(),
                    output_flat.size(),
                    INT64_MIN,
    ...
    SparseMaxVertexPoolLauncher(P_indices.flat<int64>().data(),
    ...
    auto threadFunc = [&](int thread_id) {
        ...
        for (int64 f1 = thread_id; f1 < in_features; f1+=num_threads)
        {
            for(int64  edge = 0; edge < num_edges; edge++)
            {
                ...
                if (proposedMax > output(currentBatch,currentN1,f1))
                {
                    output(currentBatch,currentN1,f1) = proposedMax;
                }
            }
        }
    };
...
REGISTER_KERNEL_BUILDER(Name("SparseMaxVertexPool").Device(DEVICE_CPU), SparseMaxVertexPoolOp<CPUDevice, T>);
...
#define REGISTER_GPU_KERNEL(T)                                     \
  REGISTER_KERNEL_BUILDER(Name("SparseMaxVertexPool") \
                              .Device(DEVICE_GPU).HostMemory("new_v_shape"),          \
                          SparseMaxVertexPoolOp<GPUDevice, T>);
