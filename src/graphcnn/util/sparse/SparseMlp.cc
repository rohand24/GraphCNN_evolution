...
bool SparseMLPLauncher(const int64 * indices,
                        const float * values,
...
class SparseMLPOp : public OpKernel {
 public:
  explicit SparseMLPOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& H_indices = context->input(1);
    const Tensor& H_values = context->input(2);
    ...
    const int64 in_features = Vin.dim_size(2);
    const int num_edges = H_indices.dim_size(0);
    const int out_num_vertices = shape(1);

    #if GOOGLE_CUDA

    SparseMLPLauncher(H_indices.flat<int64>().data(),
...
REGISTER_KERNEL_BUILDER(Name("SparseMlp").Device(DEVICE_CPU), SparseMLPOp<CPUDevice, T>);
...
#define REGISTER_GPU_KERNEL(T)                                     \
  REGISTER_KERNEL_BUILDER(Name("SparseMlp") \
